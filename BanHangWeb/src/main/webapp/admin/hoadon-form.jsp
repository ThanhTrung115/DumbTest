    ...
    <div class="admin-wrapper">
        ...
        <main class="admin-content">
            <div class="content-card">
                 <h2>${empty hd ? 'Thêm mới Hóa đơn' : 'Cập nhật Hóa đơn'}</h2>
                 <form method="post" action="${pageContext.request.contextPath}/admin/hoadon">
                    <input type="hidden" name="MaHD" value="${hd.maHD}"/>
                    <div class="form-group"><label>Mã khách hàng:</label><input class="form-control" name="MaKH" type="number" value="${hd.maKH}" required/></div>
                    <div class="form-group"><label>Ngày đặt hàng:</label><input class="form-control" name="NgayDatHang" type="date" value="${hd.ngayDatHang}" required/></div>
                    <button type="submit" class="btn btn-primary"><i class="fa-solid fa-floppy-disk"></i> Lưu lại</button>
                    <a href="${pageContext.request.contextPath}/admin/hoadon" class="btn btn-cancel"><i class="fa-solid fa-xmark"></i> Hủy</a>
                 </form>
            </div>
        </main>
    </div>
